...
{
	//printf("Inside AbsoluteLong");
	uint32	addr = READ_3WORD(CPU.PCBase + Registers.PCw);
	//printf("AbsoluteLong: Done with READ_3WORD");
	AddCycles(CPU.MemSpeedx2 + CPU.MemSpeed);
	//printf("AbsoluteLong: Done with AddCycles");
	if (a & READ)
		OpenBus = addr >> 16;
	Registers.PCw += 3;
	//printf("Done with AbsoluteLong");
	return (addr);
}
...
{
	//printf("Inside AbsoluteLongIndexedX");
